...
public interface CommentRepository extends CrudRepository<Comment, Long> {
    ...
    @Query("SELECT comments.id, comments.user_id, user.name AS user_name, comments.content, FROM comments INNER JOIN user ON comments.user_id = user.id WHERE comments.id = :id")
